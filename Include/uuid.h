...
constexpr inline unsigned char hex2char(TChar const ch)
{
    return (ch >= static_cast<TChar>('0') && ch <= static_cast<TChar>('9')) ?
        static_cast<unsigned char>(ch - static_cast<TChar>('0')) :
        (
            (ch >= static_cast<TChar>('a') && ch <= static_cast<TChar>('f')) ?
            static_cast<unsigned char>(10 + ch - static_cast<TChar>('a')) :
            (
                (ch >= static_cast<TChar>('A') && ch <= static_cast<TChar>('F')) ?
                    static_cast<unsigned char>(10 + ch - static_cast<TChar>('A')) : 0
            )
        );
}
...
public:
    ...
    {
        return uuid_iterator(&data[0], state_size);
    }
...
public:
    ...
    uuid operator()(std::wstring name)
    {
        reset();
        process_characters(name.data(), name.size());
        return make_uuid();
