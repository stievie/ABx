...
bool BanManager::IsIpDisabled(uint32_t clientIP)
{
    ...
    if ((it->second.numberOfLogins >= loginTries_) &&
        (uint32_t)currentTime < it->second.lastLoginTime + loginTimeout_)
        return true;
...
void BanManager::AddLoginAttempt(uint32_t clientIP, bool success)
{
    ...
    if (!success || (currentTime < it->second.lastLoginTime + retryTimeout_))
