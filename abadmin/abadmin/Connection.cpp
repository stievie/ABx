...
void Connection::Connect(const std::string& host, uint16_t port, const std::function<void()>& connectCallback)
{
    ...
    readTimer_.async_wait(std::bind(&Connection::OnTimeout, shared_from_this(), std::placeholders::_1));
}
...
void Connection::InternalWrite()
{
    ...
    writeTimer_.async_wait(std::bind(&Connection::OnTimeout, shared_from_this(), std::placeholders::_1));
}
...
void Connection::Write(uint8_t* buffer, size_t size)
{
    ...
    if (!outputStream_)
    {
        ...
        delayedWriteTimer_.cancel();
        delayedWriteTimer_.expires_from_now(std::chrono::milliseconds(0));
...
/*    asio::read(socket_, inputStream_.prepare(bytes));
    const char* header = asio::buffer_cast<const char*>(inputStream_.data());
    recvCallback_((uint8_t*)header, (uint16_t)inputStream_.size()); */
