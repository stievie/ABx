...
bool IOAccount::LoginServerAuth(const std::string & name, const std::string & pass, Account& account)
{
    ...
    account.id_ = result->GetLong("id");
    account.name_ = result->GetString("name");
    account.type_ = static_cast<AccountType>(result->GetInt("type"));
    account.characters_.clear();

    query.str("");
    query << "SELECT `name` FROM `players` WHERE `deleted` = 0 AND `account_id` = " << account.id_;
    for (result = db->StoreQuery(query.str()); result; result = result->Advance())
    {
        AccountCharacter character{ result->GetULong("id"), result->GetString("name") };
